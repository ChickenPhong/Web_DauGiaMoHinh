    ...
    public void doFilter(ServletRequest sr, ServletResponse sr1, FilterChain fc) throws IOException, ServletException {
        ...
        if (uri.startsWith(httpRequest.getContextPath() + "/api/secure")) {
            ...
            try {
                ...
                String taiKhoan = JwtUtils.validateTokenAndGetUsername(token);
                String vaiTro = JwtUtils.getRoleFromToken(token);
                
                if (taiKhoan != null && vaiTro != null) {
                    httpRequest.setAttribute("taiKhoan", taiKhoan);
                    httpRequest.setAttribute("role", vaiTro);
                    Set<GrantedAuthority> authorities = new HashSet<>();
                        authorities.add(new SimpleGrantedAuthority(vaiTro));
                    UsernamePasswordAuthenticationToken authentication = new UsernamePasswordAuthenticationToken(taiKhoan, null, authorities);
